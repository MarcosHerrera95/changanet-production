...
const CotizacionesPendientes = () => {
  ...
  const [cotizaciones, setCotizaciones] = useState([
    {
      id: 1,
      cliente: { nombre: 'Juan Pérez', foto: null }, // null para probar fallback
    ...
    {
      id: 2,
      cliente: { nombre: 'María García', foto: 'https://example.com/avatar.jpg' },
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.title}>Cotizaciones Pendientes</h2>
      <div style={styles.list}>
        {cotizaciones.map(cotizacion => (
          <div key={cotizacion.id} style={styles.card}>
            <div style={styles.perfil}>
              {cotizacion.cliente.foto ? (
                <img
                  src={cotizacion.cliente.foto}
                  alt={`Foto de perfil de ${cotizacion.cliente.nombre}`}
                  style={styles.avatar}
                  onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }}
                />
              ) : null}
              <div style={{ ...styles.avatar, display: cotizacion.cliente.foto ? 'none' : 'flex' }}>?</div>
